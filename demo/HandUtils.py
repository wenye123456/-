...
class HandDetector():
  ...
  def isdown(self):
    ...
    for fingers,direction in zip(fingers_list,directions):
        tip2=self.position[direction].get(8,None)
        knot2=self.position[direction].get(5,None)
        dip2=self.position[direction].get(11,None)
        if tip2[1]>knot2[1]:
          if Get_Distance(tip2,knot2)>Get_Distance(dip2,knot2):
            _,angle_t=Get_Angle(knot2,tip2,(0,-1))
            if abs(angle_t)<20:
              self.event_count+=1
              if self.event_count>=3:
  ...
  def isshoot(self)->bool:
    ...
    for fingers,direction in zip(fingers_list,directions):
      ...
      if not any(fingers[1:]):
        ...
        wrist=self.position[direction].get(0,None)
        mcp5=self.position[direction].get(17,None)
        tip2=self.position[direction].get(8,None)
        mcp2=self.position[direction].get(5,None)

        if 2*Get_Distance(tip2,mcp2)<Get_Distance(wrist,mcp5):#确保手势为握拳
          self.shoot_count+=1
          if self.shoot_count>=4:
