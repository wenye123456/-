...
class MainGame():
  def __init__(self):
    ...
    pygame.image.save(pygame.transform.scale(MainGame.background_image,(WINDOW_WIDTH,WINDOW_HEIGHT)),"1.png")
  ...
  def render(self):
    ...
    if MainGame.currentpage==0:
      ...
      for ball in MainGame.balllist:
        x=int(ball.pos['x'])
        y=int(ball.pos['y'])
        pygame.draw.circle(surface=MainGame.window,center=(x,y),radius=ball.radius,color=ball.color)
      ...
      # for line in MainGame.linelist:
      #   pygame.draw.line(surface=MainGame.window,start_pos=line.pos1,end_pos=line.pos2,
      #                  color=line.color,width=5)
      ...
      if event.type==pygame.KEYUP:
        if MainGame.currentpage==1:
          if event.key==pygame.K_r:
            ...
            MainGame.balllist.append(Ball(WINDOW_WIDTH/2,WINDOW_HEIGHT/2,v=(0,280),color=(255,0,255)))
  ...
  def update(self):
    if MainGame.currentpage!=0:
      return
    for ball in MainGame.balllist:
      if ball.CheckLine(MainGame.line):
        ball.Ricochet(MainGame.line)
      ball.Bounce()
      ball.Move()
    ...
    for ball in MainGame.balllist:
      t=pygame.sprite.spritecollide(ball,MainGame.objectgroup,False)
      if len(t)>0:
        MainGame.balllist.remove(ball)
        pass
    #time
    second=(int)((pygame.time.get_ticks()-MainGame.starttime)/1000)
    min,sec=divmod(second,60)
    MainGame.time="{0}:{1}".format(min,sec)
    if len(MainGame.balllist)==0:
      ...
      if MainGame.count==CAREMA_COUNT:
        ...
        if flag:
          ...
          if pos1!=None and pos2!=None:
            ...
            self.CreateLine(start_pos=MainGame.thumbpos,end_pos=MainGame.indexfingerpos)
